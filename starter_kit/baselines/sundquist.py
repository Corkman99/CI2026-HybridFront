...
class SundquistNetwork(torch.nn.Module):
    ...
    def forward(
        ...
        """
        level_cloud_cover = self.forward_vertical(input_level, input_auxiliary)
        total_cloud_cover = 1 - torch.prod(level_cloud_cover, dim=2)
    ...
    def forward_vertical(
        ...
        r"""
        Compute per-level cloud cover from level inputs.
        ...
        -------
        torch.Tensor
            Per-level cloud cover in [0, 1], shape (B, L, H, W).
        """
        ...
        return level_cloud_cover * valid_mask
